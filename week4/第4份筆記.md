...
int main(void)
{
    ...
    BallTracker basketball_tracker = BallTracker();
    BallFollower basketball_follower = BallFollower();

    BallTracker basket_tracker = BallTracker();
    BallFollower basket_follower = BallFollower();

    /////////////tracker and follower setting //////////////////////////

    int _marker_found = 0;
    int _ball_found = 0;
    int _basketball_found = 0 ;
    int _basket_found = 0 ;
 ...
 //////////////////// Framework Initialize ////////////////////////////
    ...
    while(1)
    {
        ...
        int greenCount = 0, blueCount = 0 , basketballcount = 0 , basketcount = 0 ;
        //created 2 int to store filtered pixel counts
        Point2D green, blue, center , basketball , basket ;
        ...
        else if(StatusCheck::m_cur_mode == NEW_MODE) //under a new mode called HONOR
        {
            basketball = ball_finder->GetPosition(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame, basketballcount);
            basket = red_finder->GetPosition(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame, basketcount);

            _basketball_found = basketball_tracker.SearchAndTracking(ball_finder->GetPosition(LinuxCamera::GetInstance()->fbuffer->m_HSVFrame));
            //use camera tracking and raise the flag if marker is found
        }
  ...
  //implement new codes here to execute after pressing start button
            ...
            if(Action::GetInstance()->IsRunning() == 0)
            {
                ...
                if(_basketball_found == 1)
                {
                basketball_follower.Process(basketball_tracker.ball_position);
                //use BallFollower object to start following marker

                        if(basketballcount >= 20000 )
                        //pixel count can be used to approximate distance, but in this case,
                        //this stop condition is sufficient
                        {
                            Walking::GetInstance()->Stop();
                            while(Walking::GetInstance()->IsRunning() == true) usleep(1000000);
                            Action::GetInstance()->m_Joint.SetEnableBody(true, true);
                                //Comment by Yu Hang: You may need this command to enable joint for Action module

                            Action::GetInstance()->Start(66);
                            while (Action::GetInstance()->IsRunning() == true) usleep(8000);
                                //Comment by Yu Hang: You should use a sleep function after performing an action to make sure op2 finish moving
                        ...
                        if(_basket_found == 1 && finish_grab == 100)
                        {
                            basket_follower.Process(basket_tracker.ball_position);
                            //use BallFollower object to start following marker
                            if(basketcount >= 10500)

                            {
                                Walking::GetInstance()->Stop();
                                while(Walking::GetInstance()->IsRunning() == true) usleep(1000000);
                                //--------------------------through---------------------------------------
                                Action::GetInstance()->Start(67);
                                while (Action::GetInstance()->IsRunning() == true) usleep(8000);
                                //Comment by Yu Hang: You should use a sleep function after performing an action to make sure op2 finish moving
                        ...
                        else if (_basket_found != 1 && finish_grab == 100)
                            {
                                Walking::GetInstance()->X_MOVE_AMPLITUDE = 1.0;
                                Walking::GetInstance()->A_MOVE_AMPLITUDE = 0.0;
                            }                       
